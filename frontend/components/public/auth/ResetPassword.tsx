"use client";
import { RESET_PASSWORD_REQUIRED_FIELDS } from "@/constants/authForm";
...
const ResetPassword = ({ token }: { token: string }) => {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await resetPassword(new FormData(e.currentTarget), token);
    } catch (error) {
        ...
        <div>
          <form onSubmit={onSubmit} onChange={handleFormChange}>
            <div className="flex flex-col gap-1.5">
              ...
              <DarkInput id="password" type="password" name="password" />
            </div>
